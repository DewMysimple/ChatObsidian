...
describe("QuickSwitcher", () => {
  ...
  it("searches note titles and shows their vault context", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("Agent 设计")).toBeInTheDocument(),
    );
    expect(
      screen.getByText(/ChatAgent \/ 项目\/Agent 设计\.md/),
    ).toBeInTheDocument();
  });
  ...
  it("keeps an actually open vault above a newer closed record", () => {
    const vaults = structuredClone(mockDashboard.vaults).map((vault) => ({
      ...vault,
      isOpen: false,
    }));
    vaults[0] = {
      ...vaults[0],
      displayName: "Closed but newer",
      lastOpened: Date.now(),
    };
    vaults[1] = {
      ...vaults[1],
  ...
  it("shows the number of available vaults beside the vault filter", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: `仓库，共 ${mockDashboard.vaults.length} 个`,
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
    ...
    ).toHaveTextContent(String(mockDashboard.vaults.length));
  });
  ...
  it("filters results by the two remembered type switches", async () => {
    ...
    expect(screen.getByRole("button", { name: /仓库，共/ })).toHaveAttribute(
      "aria-pressed",
      "false",
    );
    await user.type(screen.getByLabelText("搜索仓库或笔记标题"), "Agent");
  ...
  it("does not truncate a vault list longer than ten rows", () => {
    const vaults = [
      ...structuredClone(mockDashboard.vaults),
